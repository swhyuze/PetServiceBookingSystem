    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:cum_problem:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:cum_problem:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:cum_problem:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:cum_problem:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="cum_problemList" @selection-change="handleSelectionChange">
...
<script>
import { listCum_problem, getCum_problem, delCum_problem, addCum_problem, updateCum_problem } from "@/api/system/cum_problem";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCum_problem(this.queryParams).then(response => {
        this.cum_problemList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getCum_problem(proid).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.proid != null) {
            updateCum_problem(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCum_problem(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/cum_problem/export', {
        ...this.queryParams
      }, `cum_problem_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
